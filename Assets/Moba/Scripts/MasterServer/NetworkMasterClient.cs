...
public class NetworkMasterClient : MonoBehaviour
{
	...
	public delegate void OnJoinedRoomEvent(MasterMsgTypes.Room room);
	public OnJoinedRoomEvent onJoinedRoomEvent;

	UserLoginPanel mUserLoginPanel;
	...
	void Start(){
		mUserLoginPanel = FindObjectOfType<UserLoginPanel> ();
		mUserLoginPanel.login.onClick.Add (new EventDelegate (UserLogin));
		mUserInfoPanel = FindObjectOfType<UserInfoPanel> ();
		mUserInfoPanel.readyBtn.onClick.Add (new EventDelegate (RequestOnlinePlay));
		InitializeClient ();
	}
	...
	public void InitializeClient()
	{
		...
		// application msgs
		client.RegisterHandler (MasterMsgTypes.UserLoginedId, OnUserLogined);//鐢ㄦ埛鐧诲綍鍙嶉
		client.RegisterHandler (MasterMsgTypes.ResponseBattleServerPortMessageId, OnRequesedOnlinePlay);//璇锋眰鍦ㄧ嚎鐜╁鎴愬姛
	...
	public void UserLogin(){
		string userName = mUserLoginPanel.userName.value.Trim ();
		string password = mUserLoginPanel.password.value;
		MasterMsgTypes.UserLoginMessage msg = new MasterMsgTypes.UserLoginMessage ();
		msg.userName = userName;
		msg.password = password;
		client.Send (MasterMsgTypes.UserLoginId,msg);
	}

	//鐢ㄦ埛鐧诲綍鍙嶉
	void OnUserLogined(NetworkMessage netMsg){
		var msg = netMsg.ReadMessage<MasterMsgTypes.UserLoginedMessage>();
		if(msg.isLogin)
		{
			mUserLoginPanel.root.SetActive (false);
		}
	}
	...
	//鏀跺埌鐢ㄦ埛鍒楄〃
	void OnRequestedUserList(NetworkMessage netMsg){
//		var msg = netMsg.ReadMessage<MasterMsgTypes.UserListMessage> ();
	}


	public void RequestOnlinePlay(){
		mUserInfoPanel.readyBtn.isEnabled = false;
		mUserInfoPanel.info.gameObject.SetActive (true);
		client.Send (MasterMsgTypes.UserRequestOnlinePlayId, null);

	}

	public void OnRequesedOnlinePlay(NetworkMessage netMsg){
	...
	void OnJoinedRoom(NetworkMessage netMsg)
	{
		var msg = netMsg.ReadMessage<MasterMsgTypes.JoinedRoomMessage>();
		if(msg.joined)
		{
			if(onJoinedRoomEvent!=null)
				onJoinedRoomEvent(msg.room);
		}
	}
	...
	public void RequestJoinRoom(int roomId){
		var msg = new MasterMsgTypes.JoinRoomMessage ();
		msg.roomId = roomId;
		client.Send (MasterMsgTypes.JoinRoom,msg);
	}

	public void RequestLaunchBattle(){
		client.Send (MasterMsgTypes.LaunchBattle,new MasterMsgTypes.LaunchBattleMessage());
	}

	public void OnBattleLaunched(NetworkMessage netMsg){
...
//		}
//
//		if(GUI.Button(new Rect(100, 100, 200, 20), "Start"))
//		{
