...
namespace BlueNoah.AI.FSM
{
    ...
    public class FiniteStateMachineBehaviourEditor : UnityEditor.Editor
    {
        ...
        void DrawFiniteStateMachine(FiniteStateMachine finiteStateMachine){
            for (int i = 0;i < finiteStateMachine.stateNameList.Count;i ++){
                if (finiteStateMachine.stateNameList[i] == finiteStateMachine.CurrentState)
                    GUI.color = Color.green;
                EditorGUILayout.BeginHorizontal();
                FSMState state = finiteStateMachine.GetState(finiteStateMachine.stateNameList[i]);
                DrawState(state);
                GUI.color = Color.white;
